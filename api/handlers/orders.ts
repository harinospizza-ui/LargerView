...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const phoneDigits = order.customerPhone ? cleanPhone(order.customerPhone) : '';
      if (phoneDigits) {
        const blockedRef = db.collection('blocked_customers').doc(phoneDigits);
        const blockedSnap = await blockedRef.get();
        if (blockedSnap.exists) {
          res.status(403).json({ success: false, message: 'Forbidden. This mobile number is permanently blocked.' });
          return;
        }
      }
      ...
      if (order.customerId) {
        ...
        if (customerSnap.exists) {
          ...
          if (customerData.status === 'blocked') {
            res.status(403).json({ success: false, message: 'Forbidden. Blocked customer cannot place orders.' });
            return;
          }
        }
      }
